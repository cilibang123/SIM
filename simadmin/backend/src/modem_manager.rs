...
mod tests {
    ...
    #[test]
    fn detects_modemmanager_without_extended_signal_capabilities() {
        assert!(is_mmcli_extended_signal_unsupported(
...
fn is_mmcli_extended_signal_unsupported(message: &str) -> bool {
    message
        .to_ascii_lowercase()
        .contains("no extended signal capabilities")
}
...
async fn get_cells_data_mmcli_fallback(
    ...
    let signal_output = match read_mmcli_signal_output().await {
        Ok(output) => output,
        Err(err) if is_mmcli_extended_signal_unsupported(&err) => String::new(),
        Err(err) => return Err(zbus::fdo::Error::Failed(err).into()),
    };
    ...
    let access_tech = get_property(conn, modem_path, MM_MODEM, "AccessTechnologies")
        .await
        .ok()
        .map(|value| mm_access_tech_to_string(extract_u32(&value)))
        .unwrap_or_default();
    let tech = if signal_section.trim().is_empty() {
        mmcli_signal_section_to_tech(&access_tech).to_string()
    } else {
        mmcli_signal_section_to_tech(&signal_section).to_string()
    };
    let current_bands =
