...
use crate::config::{AutomationTask, AutomationTrigger, AutomationAction};
use crate::automation::tasks::TaskRegistry;
use crate::notification::AutomationEvent;
use crate::db::beijing_sms_now_string;
use chrono::{DateTime, Duration, FixedOffset, Datelike, Utc, NaiveDateTime, TimeZone};
        ...
        loop {
            ...
            for task in config.tasks {
                ...
                let should_trigger = match &task.trigger {
                    ...
                    AutomationTrigger::Interval { interval_value, interval_unit } => {
                        // 查询上一次运行历史
                        ...
                        match last_log {
                            Some(log) => {
                                if let Ok(parsed) = NaiveDateTime::parse_from_str(&log.created_at, "%Y-%m-%d %H:%M:%S") {
                                    let last_run_time = beijing_offset().from_local_datetime(&parsed).unwrap();
                                    let now = beijing_now();
            ...
            let retention_days = if cleanup.retention_days_enabled { Some(cleanup.retention_days) } else { None };
            let max_entries = if cleanup.max_entries_enabled { Some(cleanup.max_entries) } else { None };
            if retention_days.is_some() || max_entries.is_some() {
                let _ = app.database.cleanup_automation_logs(retention_days, max_entries);
            }
        }
    });
}

async fn execute_task(app: &AppState, registry: &TaskRegistry, task: &AutomationTask) -> Result<()> {
    ...
    let handler = match registry.get(task_type) {
        ...
        None => {
            ...
            let _ = app.database.insert_automation_log(&task.id, &task.name, task_type, "failed", &err_msg);
    ...
    let params = match &task.action {
        ...
        AutomationAction::SendSms { phone_number, content, random_delay_seconds, retry_limit } => {
    ...
    let (status, detail) = match result {
        Ok(Ok(_)) => {
            ("success", "执行成功".to_string())
        }
        Ok(Err(e)) => {
            ("failed", format!("执行失败: {}", e))
        }
        Err(_) => {
            ("failed", "执行超时 (超过60秒限制)".to_string())
        }
    };

    // 1. 写入 SQLite 日志表
    let _ = app.database.insert_automation_log(&task.id, &task.name, task_type, status, &detail);
    ...
    if let Err(e) = app.notification_sender.forward_automation_event(&event).await {
