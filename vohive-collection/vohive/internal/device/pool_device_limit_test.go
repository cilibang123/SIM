...
func TestFreeDeviceLimitIsUnlimitedByDefault(t *testing.T) {
	if DefaultFreeDeviceLimit != 0 {
		t.Fatalf("DefaultFreeDeviceLimit = %d, want 0 for unlimited", DefaultFreeDeviceLimit)
	}
	if FreeDeviceLimitReached(10_000) {
		t.Fatal("FreeDeviceLimitReached() = true, want false for unlimited default")
	}
	if !FreeDeviceLimitAllowsConfiguredDevice([]config.DeviceConfig{{ID: "dev1"}}, "dev999") {
		t.Fatal("FreeDeviceLimitAllowsConfiguredDevice() = false, want true for unlimited default")
	}
}
